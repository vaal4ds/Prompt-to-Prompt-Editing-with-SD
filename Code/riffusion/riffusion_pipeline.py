...
class RiffusionPipeline(DiffusionPipeline):
    ...
    def load_checkpoint(
        ...
        # 1. Load a standard Stable Diffusion pipeline first
        ...
        # 2. Create the RiffusionPipeline from the components of the loaded pipeline
        pipeline = cls(
            vae=stable_diffusion_pipe.vae,
            text_encoder=stable_diffusion_pipe.text_encoder,
            tokenizer=stable_diffusion_pipe.tokenizer,
            unet=stable_diffusion_pipe.unet,
            scheduler=stable_diffusion_pipe.scheduler,
            safety_checker=stable_diffusion_pipe.safety_checker,
            feature_extractor=stable_diffusion_pipe.feature_extractor,
        )

        if channels_last:
            pipeline.unet.to(memory_format=torch.channels_last)

        # Optionally load a traced unet (this part remains the same)
    ...
    def riffuse(
        ...
        # MODIFIED: Changed generator device to self.device for MPS compatibility
